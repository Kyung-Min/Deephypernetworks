function tot_nHE = runHN(episode_name, tot_nHE)
    ...
    run 'configure';
    dir_path = ['/home/kmkim/Documents/Project/AAAI2016/HN/Sampling_policy' num2str(SAMPLING_POLICY) '/'];
    mkdir(dir_path);
    ...
    for iter=1:ITER
        ...
        if iter == 1
            fprintf('Sampling HE...');
            for i=1:size(tmp_bundle, 1)
                [pop, tot_nHE] = makeHyperedge(tmp_pair{i, 2}, tmp_bundle(i, 1), tmp_presence(i, :), content_name, episode_name, tot_nHE, i);  % 하이퍼에지 만들고
                ...
                nHE(i, 1) = size(pop, 1); 
            endypernetwork for each episode');
